...
const KNOWN_KINDS = [
  ...
  { folder: 'locations', label: 'Location', desc: 'A place — city, dungeon, landmark', kbd: '2' },
  {
    folder: 'factions',
    label: 'Faction',
    desc: 'An organisation — church, guild, cabal',
    kbd: '3',
  },
...
export function QuickAdd({
  ...
  function onKey(e: React.KeyboardEvent<HTMLInputElement>) {
    ...
    if (step === 0) {
      ...
      if (e.key === 'Enter') {
        e.preventDefault();
        if (filtered[selected]) pickKind(filtered[selected]);
        return;
      }
    } else {
      if (e.key === 'Enter') {
      ...
      <div className="cmdk" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {step === 1 && folder && (
          <div className="cmdk-list">
            ...
            <div
              className="cmdk-row"
              style={{ '--kind-color': color, cursor: 'default' } as React.CSSProperties}
            >
              <span className="cmdk-row-pip" />
              <div className="cmdk-row-main">
                <div className="cmdk-row-action">
                  {folder}/<b>{slugify(title) || '…'}</b>.md
                </div>
                <div className="cmdk-row-meta">
                  A new {selectedKind?.label.toLowerCase() ?? folder} will open in a tab
                </div>
              </div>
            </div>
          </div>
